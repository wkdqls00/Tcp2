...
public class Genre_RankDAO {
	...
	public ArrayList<Genre_RankDTO> selectGenre_RankDTO(int genre_idx, String date) {
		...
		String sql = 
				"SELECT r.r_date, p.name,TO_CHAR(start_date,'YYYY-MM-DD'), TO_CHAR(end_date,'YYYY-MM-DD'), r.booking_rate, poster_url, ph.name, p.play_idx "
	  ...
	  try {
		while (rs.next()) {
				...
				Genre_RankDTO genre_Rank = new Genre_RankDTO(regDate, playName, playStartDate, playEndDate, playBookRate, poster_URL, playHallName, play_idx);
				list.add(genre_Rank);
...
public ArrayList<Week_RDTO> weekRDto(int rowNum) {
		...
		String sql = "SELECT play_idx, booking_rate "
				   + "FROM  (SELECT play_idx, booking_rate "
				   + "       FROM rank "
				   + "       WHERE TO_CHAR(r_date, 'YYYYMMDD') = '20240416' "
